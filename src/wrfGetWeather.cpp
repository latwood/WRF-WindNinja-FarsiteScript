...
bool wrfGetWeather::getWeather()
{
    ...
    projectionRef = poDS->GetProjectionRef();
    ...
    if(success == true)
    {
        ...
        for(size_t wrfFileIdx = 0; wrfFileIdx < wrf_var_list.size(); wrfFileIdx++)
        {
          ...
          // now finally open the dataset in gdal for processing :)
                    ...
                    * 6 = Lat/Long */
                    ...
                    if(mapProj == 1)    //lambert conformal conic
                    {
                        projString = "PROJCS[\"WGC 84 / WRF Lambert\",GEOGCS[\"WGS 84\",DATUM[\"World Geodetic System 1984\",\
                                      ...
                                      PARAMETER[\"central_meridian\","+std::to_string(standLon)+"],\
                                      PARAMETER[\"latitude_of_origin\","+std::to_string(moadCenLat)+"],\
                                      PARAMETER[\"standard_parallel_1\","+std::to_string(trueLat1)+"],\
                                      PARAMETER[\"false_easting\",0.0],PARAMETER[\"false_northing\",0.0],\
                                      PARAMETER[\"standard_parallel_2\","+std::to_string(trueLat2)+"],\
                    ...
                    if(success == true)
                    {
                        ...
                        char *srcWKT = NULL;
                        const char* prj2 = (const char*)projString.c_str();
                        ...
                        psWarpOptions->padfDstNoDataImag =
                            (double*) CPLMalloc( sizeof( double ) * nBandCount );   // allocate the needed data slots
                        for( size_t psWarpBandIdx = 0; psWarpBandIdx < srcDS->GetRasterCount(); psWarpBandIdx++ )
                        {
                        ...
                        wrpDS = (GDALDataset*) GDALAutoCreateWarpedVRT( srcDS, srcWKT, projectionRef.c_str(), GRA_NearestNeighbour, 1.0, psWarpOptions );
            ...
            if(success == false)    // this is necessary cause loop inside of loop stuff
            {
                break;
            }
        }
...
bool wrfGetWeather::findWrfCenterValue(GDALDataset *poDS, int band, double &foundValue)
{
    ...
    // do I need this? I think I'm good with nXSize and nYSize for what I'm doing
    ...
    {
        printf("no valid values in band!\n");
        success = false;
    }
