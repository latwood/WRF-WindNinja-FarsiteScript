...
### before doing anything else (even setting all the normal setup variables), all the dependency libraries are downloaded using "sudo apt install". This means the user will have to supply a password and approve the installations. This script used to have a bunch of nice password injection stuff to try to avoid putting in your password so many times, but this failed to work effectively, so it has been scrapped.
...
if [ $success == 0 ]; then
  ...
  echo ""  # want a nice clean line
  echo "this script assumes you have already installed git and run \"sudo apt update\" and \"sudo apt upgrade\". Expect the script to ask for password and installation approvals periodically, especially when the script reaches the WindNinja installation step, as WindNinja runs \"sudo apt install\" for some more packages."
  echo ""  # want a nice clean line
fi
...
if [ $success == 0 ]; then
  ...
  success=$?
  if [ $success != 0 ]; then
    echo "!!!error running \"sudo apt-get install "$aptgetInstallString"\"!!!"
    success=1
  fi
fi
...
if [ $success == 0 ]; then
  ...
  if [ ${baseDirName} != "include" ] && [ ${baseDirParentName} != "WRF-WindNinja-FarsiteScript" ]; then
    echo "!!!buildEverything_Ubuntu.sh is not being run from the location of the executable!!!"
    success=1
  fi
  echo ""  # want a nice clean line
fi
